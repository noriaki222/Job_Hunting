...
TestCamera::TestCamera() : m_state(CAM_DCC_NONE), m_oldPos{ 0, 0 }
{
}
...
void TestCamera::UpdateState()
{
	TestCameraKind prev = (TestCameraKind)m_state;
...
void TestCamera::UpdateOrbit(Argument& arg)
{
...
void TestCamera::UpdateTrack(Argument& arg)
{
...
void TestCamera::UpdateDolly(Argument& arg)
{
	float rate = arg.focus / m_fFarZ;
...
void TestCamera::UpdateFlight(Argument& arg)
{
	// マウスの移動量 / 画面サイズ の比率から、画面全体でどれだけ回転するか指定する。
	float angleX = 360.0f * arg.mouseMove.x / 1280.0f;
	float angleY = 180.0f * arg.mouseMove.y / 720.0f;
